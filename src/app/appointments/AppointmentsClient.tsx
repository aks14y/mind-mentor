...
export default function AppointmentsClient({ session }: AppointmentsClientProps) {
  ...
  const loadAppointments = async () => {
    try {
      ...
      toast({ title: "Failed to load", description: "Could not load appointments" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const createAppointment = async () => {
    if (!form.date || !form.time || !form.type) {
      toast({ title: "Please complete all fields", description: "All fields are required" });
      return;
    }
    try {
      setLoading(true);
      ...
      toast({ title: "Failed to create appointment", description: "Please try again" });
    } finally {
      setLoading(false);
    }
  };
          ...
          <Card className="rounded-xl shadow-lg">
            <CardContent className="p-6">
              <div className="flex flex-col space-y-4">
                ...
                <Button 
                  className="bg-brand-500 text-white hover:bg-brand-600 w-fit" 
                      ...
                      <TableBody>
                        {appointments.map((a) => (
                          <TableRow key={a.id}>
                            ...
                            <TableCell>
                              <Badge className="bg-brand-50 text-brand-700">{a.type}</Badge>
                            </TableCell>
